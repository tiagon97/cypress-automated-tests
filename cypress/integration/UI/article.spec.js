...
describe("Article creation", () => {
    beforeEach(() => {
        cy.loginToApp();
        cy.wrap("Is cypress the automation tool of the future?").as(
            "articleTitle"
        );
        cy.wrap("This article is about Cypress framework").as(
            "articleDescription"
        );
        cy.wrap(
    ...
    it("Verifies content of just created article", function () {
        ...
        onArticleDetailsPage.getArticleTags().then(function (tags) {
            ...
            tags.each(function (index) {
                expect(Cypress.$(this).text().trim()).to.eq(
                    self.articleTags[index]
                );
            });
        });
    });
